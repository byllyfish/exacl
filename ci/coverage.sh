...
if [ "$arg1" = "open" ]; then
    ...
    # shellcheck disable=SC2046
    llvm-cov show -Xdemangler=rustfilt -ignore-filename-regex='/\.cargo/' -instr-profile=coverage.profdata -show-line-counts-or-regions -show-instantiations ./target/debug/exacl $($unit_tests)
fi
